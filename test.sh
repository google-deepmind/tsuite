...
readonly VENV_DIR=/tmp/tsuite-env
rm -rf "${VENV_DIR}"
...
# Lint with flake8.
flake8 `find tsuite -name '*.py' | xargs` --count --select=E9,F63,F7,F82,E225,E251 --show-source --statistics
...
pylint --rcfile=.pylintrc `find tsuite -name '*.py' | grep -v 'test.py' | xargs` || pylint-exit $PYLINT_ARGS $?
# Disable `protected-access` warnings for tests.
pylint --rcfile=.pylintrc `find tsuite -name '*_test.py' | xargs` -d W0212 || pylint-exit $PYLINT_ARGS $?
# Cleanup.
rm .pylintrc

# Build the package.
python setup.py sdist
pip wheel --verbose --no-deps --no-clean dist/tsuite*.tar.gz
pip install tsuite*.whl
...
then
  pip install pytype
  pytype `find tsuite/_src/ -name "*py" | xargs` -k
fi;
...
# Main tests.
pytest -n "$(grep -c ^processor /proc/cpuinfo)" --pyargs tsuite
cd ..
